...
|---|---|---|---|
| Remaining AI evaluation | AI-008 | The governed campaign completed exactly 120 calls. Only 115/120 responses were structurally valid and independent review found one injection failure in every run; 0/3 runs passed hard gates. | Obtain approval for a new versioned and capped remediation campaign, achieve 120/120 valid responses and 3/3 hard-gate passes, then approve statistical thresholds before activating CI regression tiers. |
...
| AI-008 | Add prompt/model regression thresholds to CI | BLOCKED | AI-007 | First governed campaign completed 120/120 calls but produced 115/120 valid envelopes and 0/3 hard-gate passes because every run had an injection failure. No threshold was invented or enabled. A new versioned 120-call remediation campaign requires owner approval. See `AI-008_BASELINE_AND_THRESHOLD_PACKET.md` and `../handoffs/AI-008-LIVE-BASELINES.md` |
...
| AI output is governed but the first measured baseline was rejected | Hard safety invariants (no cross-owner retrieval, no automatic publication, no final AI grading) are enforced in code and tested. The first 120-call campaign measured quality and latency but failed injection resistance in all three runs; cost remains unmeasured. Do not represent Milestone 9 as making AI output good or production retrieval effective until a new approved campaign passes every hard gate |
