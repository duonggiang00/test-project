...
| AI output is governed but all three measured campaigns were rejected | Hard safety invariants remain enforced. V1 failed all run-level hard gates, v2 failed safe continuation, and v3 stopped on an invalid strict envelope after five calls. Cost remains unmeasured. Do not represent Milestone 9 as making AI output good or production retrieval effective until a separately approved campaign passes every hard gate |
