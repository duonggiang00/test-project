...
Verified on 2026-08-29, the milestone tables contain 96 `DONE`, 1 `TODO`,
...
|---|---|---|---|
| Remaining AI evaluation | AI-008 | The rejected v1 campaign used 120 calls and passed 0/3 hard gates. The approved v2 remediation stopped after its ten-call canary: format, citations, injection resistance, and the required explicit refusal passed, but safe continuation passed only 3/8 injection cases. | Obtain approval for a separately versioned v3 remediation, pass its bounded canary, achieve 120/120 valid responses and 3/3 hard-gate passes, then approve statistical thresholds before activating CI regression tiers. |
...
| AI-008 | Add prompt/model regression thresholds to CI | BLOCKED | AI-007 | V1 was rejected after 120 calls. V2 added JSON mode, pinned routing, runtime policy attestation, and a hard ten-call canary; the canary achieved 10/10 format, 10/10 citations, 8/8 injection resistance, and 1/1 required explicit refusal, but only 3/8 safe continuations. The campaign stopped at 10/120 and no threshold was enabled. A separately approved v3 campaign is required. See `AI-008_BASELINE_AND_THRESHOLD_PACKET.md`, `../handoffs/AI-008-LIVE-BASELINES.md`, and `../handoffs/AI-008-V2-REMEDIATION.md` |
...
| AI output is governed but both measured campaigns were rejected | Hard safety invariants (no cross-owner retrieval, no automatic publication, no final AI grading) are enforced in code and tested. V1 failed all three run-level hard gates; v2 improved format, citations, injection resistance, and explicit refusal in its ten-call canary but failed safe-task continuation in five injection cases and stopped before the remaining 110 calls. Cost remains unmeasured. Do not represent Milestone 9 as making AI output good or production retrieval effective until a separately approved campaign passes every hard gate |
